...
namespace litehtml {
  ...
  public:
    ...
    int add_line(int max_width, element_clear clr = clear_none,
		 int el_width = 0, bool finish_prev = true);
    ...
    virtual const wchar_t* get_attr(const wchar_t* name,
				    const wchar_t* def = 0);
    ...
    virtual bool find_styles_changes(position::vector& redraw_boxes, int x,
				     int y);
    ...
    virtual void draw_background(uint_ptr hdc, int x, int y,
				 const position* clip);
    int left() const;
    int right() const;
    int top() const;
    ...
    virtual const wchar_t* get_style_property(const wchar_t* name,
					      bool inherited, const wchar_t* def = 0);

    uint_ptr get_font();
    virtual int get_font_size();
    litehtml::web_color get_color(const wchar_t* prop_name, bool inherited,
    ...
    element* find_ancestor(const css_selector& selector, bool apply_pseudo =
			   true, bool* is_pseudo = 0);
  ...
  private:
    ...
    void init_line(line::ptr& ln, int top, int def_right,
		   element_clear el_clear = clear_none);
  ...
  inline bool element::is_floats_holder() const {
	...
	|| !m_parent || is_body() || m_float != float_none
  ...
  inline int litehtml::element::height_raw() const {
    return m_pos.height + (m_margins.top > 0 ? m_margins.top : 0)
      + (m_margins.bottom > 0 ? m_margins.bottom : 0) + m_padding.height()
      + m_borders.height();
  }
