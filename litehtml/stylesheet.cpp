...
void litehtml::css::parse_stylesheet(const wchar_t* str, const wchar_t* baseurl,
     ...
     while (pos != std::wstring::npos) {
	  ...
	  if (style_start != std::wstring::npos
	      && style_end != std::wstring::npos) {
	       style::ptr st = new style;
	       st->add(
		    text.substr(style_start + 1, style_end - style_start - 1).c_str(),
		    baseurl);
...
void litehtml::css::parse_selectors(const std::wstring& txt,
				    litehtml::style::ptr styles) {
