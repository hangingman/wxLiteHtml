...
void litehtml::style::parse(const wchar_t* txt, const wchar_t* baseurl) {
     ...
     for (std::vector<std::wstring>::const_iterator i = properties.begin();
	  i != properties.end(); i++) {
	  parse_property(*i, baseurl);
     }
}

void litehtml::style::parse_property(const std::wstring& txt,
...
void litehtml::style::combine(const litehtml::style& src) {
     for (string_map::const_iterator i = src.m_properties.begin();
	  i != src.m_properties.end(); i++) {
	  m_properties[i->first] = i->second;
     }
}

void litehtml::style::add_property(const wchar_t* name, const wchar_t* val,
				   const wchar_t* baseurl) {
     if (!name || !val) {
	  return;
     }
     ...
     if (!wcscmp(name, L"background-image")) {
	  ...
	  if (!wcscmp(name, L"border-spacing")) {
	       ...
	       if (tokens.size() == 1) {
		    add_property(L"-litehtml-border-spacing-x", tokens[0].c_str(),
				 baseurl);
		    add_property(L"-litehtml-border-spacing-y", tokens[0].c_str(),
				 baseurl);
	       } else if (tokens.size() == 2) {
		    add_property(L"-litehtml-border-spacing-x", tokens[0].c_str(),
				 baseurl);
		    add_property(L"-litehtml-border-spacing-y", tokens[1].c_str(),
				 baseurl);
	       }
	  } else
	       ...
	       if (!wcscmp(name, L"border")) {
		    ...
		    if (!wcscmp(name, L"border-bottom-left-radius")) {
			 ...
			 tokenize(val, tokens, L" ");
			 if (tokens.size() >= 2) {
			      add_property(L"border-bottom-left-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-bottom-left-radius-y", tokens[1].c_str(),
					   baseurl);
			 } else if (tokens.size() == 1) {
			      add_property(L"border-bottom-left-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-bottom-left-radius-y", tokens[0].c_str(),
					   baseurl);
			 }
			 ...
			 tokenize(val, tokens, L" ");
			 if (tokens.size() >= 2) {
			      add_property(L"border-bottom-right-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-bottom-right-radius-y", tokens[1].c_str(),
					   baseurl);
			 } else if (tokens.size() == 1) {
			      add_property(L"border-bottom-right-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-bottom-right-radius-y", tokens[0].c_str(),
					   baseurl);
			 }
			 ...
			 tokenize(val, tokens, L" ");
			 if (tokens.size() >= 2) {
			      add_property(L"border-top-right-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-top-right-radius-y", tokens[1].c_str(),
					   baseurl);
			 } else if (tokens.size() == 1) {
			      add_property(L"border-top-right-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-top-right-radius-y", tokens[0].c_str(),
					   baseurl);
			 }
			 ...
			 tokenize(val, tokens, L" ");
			 if (tokens.size() >= 2) {
			      add_property(L"border-top-left-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-top-left-radius-y", tokens[1].c_str(),
					   baseurl);
			 } else if (tokens.size() == 1) {
			      add_property(L"border-top-left-radius-x", tokens[0].c_str(),
					   baseurl);
			      add_property(L"border-top-left-radius-y", tokens[0].c_str(),
					   baseurl);
			 }
			 ...
			 if (!wcscmp(name, L"border-radius")) {
			      ...
			      if (tokens.size() == 1) {
				   ...
				   add_property(L"border-top-left-radius-x", tokens[0].c_str(),
						baseurl);
				   add_property(L"border-top-right-radius-x", tokens[1].c_str(),
						baseurl);
				   add_property(L"border-bottom-right-radius-x", tokens[0].c_str(),
						baseurl);
				   add_property(L"border-bottom-left-radius-x", tokens[1].c_str(),
						baseurl);
				   ...
				   add_property(L"border-top-right-radius-x", tokens[1].c_str(),
						baseurl);
				   add_property(L"border-bottom-right-radius-x", tokens[2].c_str(),
						baseurl);
				   add_property(L"border-bottom-left-radius-x", tokens[1].c_str(),
						baseurl);
				   ...
				   add_property(L"border-bottom-right-radius-x", tokens[2].c_str(),
						baseurl);
				   add_property(L"border-bottom-left-radius-x", tokens[3].c_str(),
						baseurl);
			      }
			      ...
			      if (tokens.size() == 1) {
				   ...
				   add_property(L"border-top-right-radius-y", tokens[1].c_str(),
						baseurl);
				   add_property(L"border-bottom-right-radius-y", tokens[2].c_str(),
						baseurl);
				   add_property(L"border-bottom-left-radius-y", tokens[1].c_str(),
						baseurl);
			      } else if (tokens.size() == 4) {
				   ...
				   add_property(L"border-bottom-right-radius-y", tokens[2].c_str(),
						baseurl);
				   add_property(L"border-bottom-left-radius-y", tokens[3].c_str(),
						baseurl);
			      }
			 }
     ...
     if (!wcscmp(name, L"list-style")) {
	  ...
	  for (string_vector::iterator tok = tokens.begin(); tok != tokens.end();
	       ...
	       if (idx >= 0) {
		    ...
		    idx = value_index(tok->c_str(), list_style_position_strings,
				      -1);
		    if (idx >= 0) {
	  ...
	  if (!wcscmp(name, L"background")) {
	       ...
	       if (!wcscmp(name, L"margin") || !wcscmp(name, L"padding")) {
		    ...
		    if (!wcscmp(name, L"border-left") || !wcscmp(name, L"border-right")
			|| !wcscmp(name, L"border-top")
			|| !wcscmp(name, L"border-bottom")) {
			 ...
			 if (!wcscmp(name, L"border")) {
			      ...
			      if (!wcscmp(name, L"border-width") || !wcscmp(name, L"border-style")
				  || !wcscmp(name, L"border-color")) {
				   ...
				   if (tokens.size() >= 4) {
					...
					m_properties[nametokens[0] + L"-right-" + nametokens[1]] =
					     tokens[1];
					m_properties[nametokens[0] + L"-bottom-" + nametokens[1]] =
					     tokens[2];
...
void litehtml::style::parse_short_border(const std::wstring& prefix,
...
void litehtml::style::parse_short_background(const std::wstring& val,
					     const wchar_t* baseurl) {
     string_vector tokens;
     tokenize(val, tokens, L" ", L"", L"()");
	  ...
	  if (web_color::is_color(tok->c_str())) {
	       ...
	       if (!origin_found) {
		    ...
		    m_properties[L"background-clip"] = *tok;
		     ...
		     || iswdigit((*tok)[0]) || (*tok)[0] == L'-' || (*tok)[0] == L'.'
		     || (*tok)[0] == L'+') {
	       if (m_properties.find(L"background-position")
		   != m_properties.end()) {
		    m_properties[L"background-position"] =
			 m_properties[L"background-position"] + L" " + *tok;
	       } else {
