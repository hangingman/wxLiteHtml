...
litehtml::el_text::el_text(const wchar_t* text, litehtml::document* doc) :
     element(doc) {
     if (text) {
	  m_text = text;
     }
...
void litehtml::el_text::draw_content(uint_ptr hdc,
				     const litehtml::position& pos) {
     uint_ptr font = m_parent->get_font();
     litehtml::web_color color = m_parent->get_color(L"color", true,
						     m_doc->get_def_color());

     if (m_text_transform == text_transform_none) {
	  m_doc->container()->draw_text(hdc, m_text.c_str(), font, color, pos);
     } else {
	  m_doc->container()->draw_text(hdc, m_transformed_text.c_str(), font,
					color, pos);
     }
}
...
const wchar_t* litehtml::el_text::get_style_property(const wchar_t* name,
...
void litehtml::el_text::parse_styles(bool is_reparse) {
     m_text_transform = (text_transform) value_index(
	  get_style_property(L"text-transform", true, L"none"),
     ...
     if (m_text_transform != text_transform_none) {
	  ...
	  case text_transform_capitalize:
	       ...
	       if (!m_transformed_text.empty()) {
		    m_transformed_text[0] = m_doc->container()->toupper(
			 m_transformed_text[0]);
	       }
	       break;
     ...
     if (m_text_transform == text_transform_none) {
	  m_size.width = m_doc->container()->text_width(hdc, m_text.c_str(),
							font);
     } else {
	  m_size.width = m_doc->container()->text_width(hdc,
							m_transformed_text.c_str(), font);
     }
     ...
     if (m_parent->get_display() == display_inline) {
	  ...
	  if (m_parent->is_last_child(this)) {
	       ...
	       m_padding.right = m_parent->padding_right()
		    + m_parent->border_right();
	  }
     }
}
