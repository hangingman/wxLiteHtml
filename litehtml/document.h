...
namespace litehtml {
  ...
  str_istream(const wchar_t* src) :
    p(src), end(src + wcslen(src)) {
    }
  ...
  public:
    ...
    uint_ptr get_font(const wchar_t* name, int size, const wchar_t* weight,
    ...
    static litehtml::document::ptr createFromString(const wchar_t* str,
						    litehtml::document_container* objPainter, litehtml::context* ctx);

  private:
    litehtml::element* add_root();
    litehtml::element* add_body();
    litehtml::uint_ptr add_font(const wchar_t* name, int size,
