...
const wchar_t* g_empty_tags[] = { L"base", L"meta", L"link", L"br", L"hr",
				  L"input", L"button", L"img", L"param", L"col", 0 };
...
litehtml::document::ptr litehtml::document::createFromString(const wchar_t* str,
							     litehtml::document_container* objPainter, litehtml::context* ctx) {
     ...
     if (doc->m_root) {
	  ...
	  for (css_text::vector::iterator css = doc->m_css.begin();
	       css != doc->m_css.end(); css++) {
	       doc->m_styles.parse_stylesheet(css->text.c_str(),
					      css->baseurl.c_str(), doc->container());
	  }
...
void litehtml::document::draw(uint_ptr hdc, int x, int y,
			      const position* clip) {
     if (m_root) {
...
int litehtml::document::cvt_units(const wchar_t* str, int fontSize,
     ...
     css_length val;
     val.fromString(str);
     if (is_percent && val.units() == css_units_percentage
	 && !val.is_predefined()) {
	  *is_percent = true;
     }
...
bool litehtml::document::on_mouse_over(int x, int y,
...
bool litehtml::document::on_lbutton_down(int x, int y,
					 position::vector& redraw_boxes) {
     if (!m_root) {
	  return false;
     }
...
bool litehtml::document::on_lbutton_up(int x, int y,
				       position::vector& redraw_boxes) {
     if (!m_root) {
	  return false;
     }
...
void litehtml::document::parse_attribute(const wchar_t* attr_name,
