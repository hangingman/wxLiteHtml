...
litehtml::el_table::el_table(litehtml::document* doc) :
     element(doc) {
...
int litehtml::el_table::render(int x, int y, int max_width) {
     ...
     for (int col = 0; col < m_grid.cols_count(); col++) {
	  for (int row = 0; row < m_grid.rows_count(); row++) {
	       if (m_grid.cell(col, row)->colspan > 1) {
		    ...
		    for (int col2 = col + 1;
			 col2 < col + m_grid.cell(col, row)->colspan; col2++) {
		    ...
		    if (min_total_width < m_grid.cell(col, row)->min_width) {
			 m_grid.distribute_min_width(
			      ...
			      col, col + m_grid.cell(col, row)->colspan - 1);
		    }
     ...
     if (!m_css_width.is_predefined()) {
	  ...
	  if (table_width > min_table_width) {
	       ...
	       m_grid.distribute_width(table_width - min_table_width, 0,
				       m_grid.cols_count()
				       - 1/*, &table_column_accessor_width()*/);
	  } else {
	       for (int col2 = 0; col2 < m_grid.cols_count(); col2++) {
		    m_grid.column(col2).width = m_grid.column(col2).min_width;
	       }
     ...
     for (int row = 0; row < m_grid.rows_count(); row++) {
	  ...
	  for (int col = 0; col < m_grid.cols_count(); col++) {
	       ...
	       if (cell->el && cell->rowspan <= 1) {
		    cell->el->m_pos.height = max_height
     ...
     if (row_span_found) {
	  ...
	  for (int row = 0; row < m_grid.rows_count(); row++) {
	       for (int col = 0; col < m_grid.cols_count(); col++) {
		    ...
		    if (cell->el) {
			 if (cell->rowspan > 1) {
			      int cell_height = (cell->rowspan - 1)
				   * m_border_spacing_y;
			      for (int row2 = row;
				   row2 < row + cell->rowspan
					&& row2 < m_grid.rows_count(); row2++) {
				   cell_height += m_grid.row(row2).height;
			      }
			      if (cell_height >= cell->el->height()) {
				   cell->el->m_pos.height = cell_height
					- cell->el->content_margins_top()
					- cell->el->content_margins_bottom();
			      } else {
				   m_grid.row(
	  ...
	  for (int col = 0; col < m_grid.cols_count(); col++) {
	       ...
	       if (cell->el) {
		    if (!cell->el->m_lines.empty()) {
			 int add = 0;
			 int content_height = cell->el->m_lines.back()->get_top()
			      + cell->el->m_lines.back()->get_height();
...
bool litehtml::el_table::appendChild(litehtml::element* el) {
     if (!el)
	  return false;
     if (el->m_tag == L"tbody" || el->m_tag == L"thead"
	 || el->m_tag == L"tfoot") {
...
void litehtml::el_table::parse_styles(bool is_reparse) {
     ...
     m_css_border_spacing_x.fromString(
	  get_style_property(L"-litehtml-border-spacing-x", true, L"0px"));
     m_css_border_spacing_y.fromString(
	  get_style_property(L"-litehtml-border-spacing-y", true, L"0px"));
...
void litehtml::el_table::draw(uint_ptr hdc, int x, int y,
			      const position* clip) {
     position pos = m_pos;
     pos.x += x;
     pos.y += y;
