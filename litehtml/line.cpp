...
void litehtml::line::add_element(element* el) {
     ...
     if (el->m_float == float_none && !el->m_skip) {
	  if (last_space) {
	       ...
	       m_height = std::max(last_space->height(), m_height);
	       m_padding_bottom = std::max(m_padding_bottom,
				      last_space->m_padding.bottom
				      + last_space->m_borders.bottom);
	       m_padding_top = std::max(m_padding_top,
				   last_space->m_padding.top + last_space->m_borders.top);
	       m_top_margin = std::max(last_space->margin_top(), m_top_margin);
	       m_bottom_margin = std::max(last_space->margin_bottom(), m_bottom_margin);
	       m_left += last_space->width();
	       last_space->m_skip = false;
	       el->m_pos.x += last_space->width();
	  }
	  m_height = std::max(el->height(), m_height);
	  m_padding_bottom = std::max(m_padding_bottom,
				 el->m_padding.bottom + el->m_borders.bottom);
	  m_padding_top = std::max(m_padding_top,
			      el->m_padding.top + el->m_borders.top);
	  m_top_margin = std::max(el->margin_top(), m_top_margin);
	  m_bottom_margin = std::max(el->margin_bottom(), m_bottom_margin);
	  m_left += el->width();
...
void litehtml::line::set_top(int top, element* parent) {
     ...
     for (elements_vector::iterator i = m_items.begin(); i != m_items.end();
	  i++) {
	  ...
	  if (el->m_float == float_none) {
	       ...
	       case va_baseline:
		    if (!is_block()) {
			 el->m_pos.y = top + m_height - m_bottom_margin
			      - parent_base_line - m_padding_bottom
			      - el->m_pos.height + el->get_base_line()
			      - m_top_margin;
			 el->m_pos.y += add;
		    } else {
		    ...
		    break;
	       case va_middle:
		    el->m_pos.y = top + std::max(m_height, m_min_height) / 2
			 - el->m_pos.height / 2;
		    break;
	       default:
		    el->m_pos.y = top + el->content_margins_top() + add;
...
bool litehtml::line::finish(text_align align) {
     ...
     bool ret = false;
     for (elements_vector::reverse_iterator i = m_items.rbegin();
	  i != m_items.rend(); i++) {
	  object_ptr < element > el = (*i);
     ...
     if (ret || m_is_break) {
	  if (!m_is_block) {
	       m_height = std::max(m_min_height, m_height);
	  }
     } else {
	  m_height = 0;
     }
...
bool litehtml::line::empty() const {
     ...
     for (elements_vector::const_iterator i = m_items.begin();
	  i != m_items.end(); i++) {
	  object_ptr < element > el = (*i);
	  if (!el->m_skip) {
	       return false;
