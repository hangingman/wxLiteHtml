...
const wchar_t* litehtml::element::get_attr(const wchar_t* name,
					   const wchar_t* def) {
...
void litehtml::element::apply_stylesheet(const litehtml::css& stylesheet) {
     for (litehtml::css_selector::vector::const_iterator sel =
	       stylesheet.selectors().begin(); sel != stylesheet.selectors().end();
	  sel++) {
     ...
     for (elements_vector::iterator i = m_children.begin();
	  i != m_children.end(); i++) {
...
void litehtml::element::draw(uint_ptr hdc, int x, int y, const position* clip) {
	 ...
	 && m_list_style_type != list_style_type_none) {
	  ...
	  litehtml::web_color color = get_color(L"color", true,
						web_color(0, 0, 0));
	  m_doc->container()->draw_list_marker(hdc, m_list_style_type, marker_x,
					       pos.y, marker_height, color);
     }
     ...
     for (elements_vector::iterator i = m_absolutes.begin();
	  i != m_absolutes.end(); i++) {
	  element* el = (*i);
	  el->draw(hdc, pos.left(), pos.top(), clip);
     }
...
const wchar_t* litehtml::element::get_style_property(const wchar_t* name,
						     bool inherited, const wchar_t* def /*= 0*/) {
...
litehtml::web_color litehtml::element::get_color(const wchar_t* prop_name,
						 bool inherited, const litehtml::web_color& def_color) {
...
void litehtml::element::draw_content(uint_ptr hdc,
...
void litehtml::element::parse_styles(bool is_reparse) {
     ...
     m_el_position = (element_position) value_index(
	  get_style_property(L"position", false, L"static"),
	  element_position_strings, element_position_fixed);
     m_text_align = (text_align) value_index(
	  get_style_property(L"text-align", true, L"left"),
	  text_align_strings, text_align_left);
     m_overflow = (overflow) value_index(
	  get_style_property(L"overflow", false, L"visible"),
	  overflow_strings, overflow_visible);
     m_white_space = (white_space) value_index(
	  get_style_property(L"white-space", true, L"normal"),
	  white_space_strings, white_space_normal);
     m_display = (style_display) value_index(
	  get_style_property(L"display", false, L"inline"),
	  style_display_strings, display_inline);

     const wchar_t* va = get_style_property(L"vertical-align", true,
					    L"baseline");
     m_vertical_align = (vertical_align) value_index(va, vertical_align_strings,
						     va_baseline);

     const wchar_t* fl = get_style_property(L"float", false, L"none");
     m_float = (element_float) value_index(fl, element_float_strings,
					   float_none);

     m_clear = (element_clear) value_index(
	  get_style_property(L"clear", false, L"none"), element_clear_strings,
	  clear_none);
     ...
     m_css_width.fromString(get_style_property(L"width", false, L"auto"),
			    L"auto");
     m_css_height.fromString(get_style_property(L"height", false, L"auto"),
			     L"auto");
     ...
     m_css_right.fromString(get_style_property(L"right", false, L"auto"),
			    L"auto");
     m_css_top.fromString(get_style_property(L"top", false, L"auto"), L"auto");
     m_css_bottom.fromString(get_style_property(L"bottom", false, L"auto"),
			     L"auto");
     ...
     m_css_margins.left.fromString(
	  get_style_property(L"margin-left", false, L"0"), L"auto");
     m_css_margins.right.fromString(
	  get_style_property(L"margin-right", false, L"0"), L"auto");
     m_css_margins.top.fromString(get_style_property(L"margin-top", false, L"0"),
				  L"auto");
     m_css_margins.bottom.fromString(
	  get_style_property(L"margin-bottom", false, L"0"), L"auto");

     m_css_padding.left.fromString(
	  get_style_property(L"padding-left", false, L"0"), L"");
     m_css_padding.right.fromString(
	  get_style_property(L"padding-right", false, L"0"), L"");
     m_css_padding.top.fromString(
	  get_style_property(L"padding-top", false, L"0"), L"");
     m_css_padding.bottom.fromString(
	  get_style_property(L"padding-bottom", false, L"0"), L"");

     m_css_borders.left.width.fromString(
	  get_style_property(L"border-left-width", false, L"medium"),
	  border_width_strings);
     m_css_borders.right.width.fromString(
	  get_style_property(L"border-right-width", false, L"medium"),
	  border_width_strings);
     m_css_borders.top.width.fromString(
	  get_style_property(L"border-top-width", false, L"medium"),
	  border_width_strings);
     m_css_borders.bottom.width.fromString(
	  get_style_property(L"border-bottom-width", false, L"medium"),
	  border_width_strings);

     m_css_borders.left.color = web_color::from_string(
	  get_style_property(L"border-left-color", false, L""));
     m_css_borders.left.style = (border_style) value_index(
	  get_style_property(L"border-left-style", false, L"none"),
	  border_style_strings, border_style_none);

     m_css_borders.right.color = web_color::from_string(
	  get_style_property(L"border-right-color", false, L""));
     m_css_borders.right.style = (border_style) value_index(
	  get_style_property(L"border-right-style", false, L"none"),
	  border_style_strings, border_style_none);

     m_css_borders.top.color = web_color::from_string(
	  get_style_property(L"border-top-color", false, L""));
     m_css_borders.top.style = (border_style) value_index(
	  get_style_property(L"border-top-style", false, L"none"),
	  border_style_strings, border_style_none);

     m_css_borders.bottom.color = web_color::from_string(
	  get_style_property(L"border-bottom-color", false, L""));
     m_css_borders.bottom.style = (border_style) value_index(
	  get_style_property(L"border-bottom-style", false, L"none"),
	  border_style_strings, border_style_none);

     m_css_borders.radius.top_left_x.fromString(
	  get_style_property(L"border-top-left-radius-x", false, L"0"));
     m_css_borders.radius.top_left_y.fromString(
	  get_style_property(L"border-top-left-radius-y", false, L"0"));

     m_css_borders.radius.top_right_x.fromString(
	  get_style_property(L"border-top-right-radius-x", false, L"0"));
     m_css_borders.radius.top_right_y.fromString(
	  get_style_property(L"border-top-right-radius-y", false, L"0"));

     m_css_borders.radius.bottom_right_x.fromString(
	  get_style_property(L"border-bottom-right-radius-x", false, L"0"));
     m_css_borders.radius.bottom_right_y.fromString(
	  get_style_property(L"border-bottom-right-radius-y", false, L"0"));

     m_css_borders.radius.bottom_left_x.fromString(
	  get_style_property(L"border-bottom-left-radius-x", false, L"0"));
     m_css_borders.radius.bottom_left_y.fromString(
	  get_style_property(L"border-bottom-left-radius-y", false, L"0"));
     ...
     m_borders.bottom = m_doc->cvt_units(m_css_borders.bottom.width,
					 m_font_size);
     ...
     if (m_display == display_list_item) {
	  const wchar_t* list_type = get_style_property(L"list-style-type", true,
							L"disc");
	  m_list_style_type = (list_style_type) value_index(list_type,
							    list_style_type_strings, list_style_type_disc);

	  const wchar_t* list_pos = get_style_property(L"list-style-position",
						       true, L"outside");
	  m_list_style_position = (list_style_position) value_index(list_pos,
								    list_style_position_strings, list_style_position_outside);
     }
...
int litehtml::element::render(int x, int y, int max_width) {
     ...
     if (!m_lines.empty()) {
	  ...
	  if (m_display == display_table_cell || is_body()) {
	       ...
	       if (!m_padding.top && !m_borders.top
     ...
     for (elements_vector::iterator abs_el = m_absolutes.begin();
	  abs_el != m_absolutes.end(); abs_el++) {
	  element* el = (*abs_el);
	  ...
	  int bottom = m_pos.height
	       - el->m_css_bottom.calc_percent(abs_base_height);
     ...
     for (elements_vector::iterator abs_el = m_absolutes.begin();
	  ...
	  } else if (!el->m_css_left.is_predefined()
		     && el->m_css_right.is_predefined()) {
	       el->m_pos.x = left - m_padding.left + el->content_margins_left();
	  } else if (el->m_css_left.is_predefined()
		     && !el->m_css_right.is_predefined()) {
	       el->m_pos.x = right - el->width() + el->content_margins_right()
		    + m_padding.right;
	  } else {
	       el->m_pos.x = left - m_padding.left + el->content_margins_left();
	       el->m_pos.width = (right + el->content_margins_right()
				  + m_padding.right) - el->m_pos.x;
	       need_render = true;
	  }

	  if (el->m_css_top.is_predefined() && el->m_css_bottom.is_predefined()) {
	       el->m_pos.y = parent_pos.top() + el->content_margins_top();
	  } else if (!el->m_css_top.is_predefined()
		     && el->m_css_bottom.is_predefined()) {
	       el->m_pos.y = top - m_padding.top + el->content_margins_top();
	  } else if (el->m_css_top.is_predefined()
		     && !el->m_css_bottom.is_predefined()) {
	       el->m_pos.y = bottom - el->height() + el->content_margins_bottom()
		    + m_padding.bottom;
	  } else {
	       el->m_pos.y = top - m_padding.top + el->content_margins_top();
	       el->m_pos.height = (bottom + el->content_margins_bottom()
				   + m_padding.bottom) - el->m_pos.y;
	       need_render = true;
	  }
...
litehtml::line::ptr litehtml::element::last_line() const {
     ...
     for (line::vector::const_reverse_iterator i = m_lines.rbegin();
	  i != m_lines.rend(); i++) {
	  if (!(*i)->empty()) {
	       ret = (*i);
	       break;
...
litehtml::line::ptr litehtml::element::first_line() const {
     ...
     for (line::vector::const_iterator i = m_lines.begin(); i != m_lines.end();
	  i++) {
	  if (!(*i)->empty()) {
	       ret = (*i);
	       break;
...
void litehtml::element::finish_line(int max_width) {
     if (!m_lines.empty()) {
	  ...
	  for (line::vector::reverse_iterator i = m_lines.rbegin() + 1;
	       i != m_lines.rend(); i++) {
	       if (!(*i)->empty()) {
		    prev_line = (*i);
		    break;
...
int litehtml::element::add_line(int max_width, element_clear clr, int el_width,
     ...
     int left = 0;
     if (m_list_style_position == list_style_position_inside
...
void litehtml::element::init() {
     for (elements_vector::iterator el = m_children.begin();
	  el < m_children.end(); el++) {
...
int litehtml::element::select(const css_selector& selector, bool apply_pseudo) {
     ...
     if (selector.m_left) {
	  ...
	  case combinator_descendant: {
	       ...
	       element* res = find_ancestor(*selector.m_left, apply_pseudo,
					    &is_pseudo);
	       if (!res) {
		    return 0;
	       } else {
...
int litehtml::element::select(const css_element_selector& selector,
			      bool apply_pseudo) {
     ...
     for (css_attribute_selector::map::const_iterator i =
	       selector.m_attrs.begin(); i != selector.m_attrs.end(); i++) {
	  ...
	  case select_equal:
	       if (!attr_value) {
		    ...
		    if (i->first == L"class") {
			 ...
			 bool found = false;
			 for (string_vector::iterator str = tokens.begin();
			      str != tokens.end() && !found; str++) {
			      if (*str == i->second.val) {
				   found = true;
			      }
			 }
	  ...
	  case select_start_str:
	       if (!attr_value) {
		    return 0;
	       } else if (wcsncmp(attr_value, i->second.val.c_str(),
				  i->second.val.length())) {
		    return 0;
	       }
	       break;
	  case select_end_str:
	       if (!attr_value) {
		    return 0;
				  ...
				  i->second.val.length())) {
		    const wchar_t* s = attr_value + wcslen(attr_value)
			 - i->second.val.length() - 1;
		    if (s < attr_value) {
			 return 0;
		    }
		    if (i->second.val != s) {
	  ...
	  case select_pseudo_class:
	       if (apply_pseudo) {
		    if (std::find(m_pseudo_classes.begin(), m_pseudo_classes.end(),
				  i->second.val) == m_pseudo_classes.end()) {
			 return 0;
		    }
	       } else {
		    pseudo_found = true;
...
litehtml::element* litehtml::element::find_ancestor(
...
int litehtml::element::get_floats_height() const {
     if (is_floats_holder()) {
	  int h = 0;
	  if (!m_floats.empty()) {
	       for (elements_vector::const_iterator i = m_floats.begin();
		    i != m_floats.end(); i++) {
		    element::ptr el = (*i);
...
int litehtml::element::find_next_line_top(int top, int width, int def_right) {
     if (is_floats_holder()) {
	  ...
	  for (elements_vector::const_iterator i = m_floats.begin();
	       ...
	       if (el_pos.top() >= top) {
		    if (find(points.begin(), points.end(), el_pos.top())
			== points.end()) {
			 points.push_back(el_pos.top());
		    }
	       }
	       if (el_pos.bottom() >= top) {
		    if (find(points.begin(), points.end(), el_pos.bottom())
			== points.end()) {
	  ...
	  if (!points.empty()) {
	       ...
	       for (int_vector::iterator i = points.begin(); i != points.end();
		    i++) {
...
void litehtml::element::parse_background() {
     ...
     const wchar_t* str = get_style_property(L"background-position", false,
					     L"0% 0%");
     if (str) {
	  string_vector res;
	  tokenize(str, res, L" \t");
	  if (res.size() > 0) {
	       if (res.size() == 1) {
		    ...
		    } else if (value_in_list(res[0].c_str(),
					     L"top;bottom;center")) {
     ...
     m_bg.m_origin = (background_box) value_index(
	  ...
	  background_box_strings, background_box_content);

     // parse background-image
     css::parse_css_url(get_style_property(L"background-image", false, L""),
			m_bg.m_image);
     m_bg.m_baseurl = get_style_property(L"background-image-baseurl", false,
					 L"");

     if (!m_bg.m_image.empty()) {
	  m_doc->container()->load_image(m_bg.m_image.c_str(),
...
void litehtml::element::get_inline_boxes(position::vector& boxes) {
     line* old_line = 0;
     position pos;
     for (elements_vector::iterator iter = m_children.begin();
...
bool litehtml::element::on_mouse_over(int x, int y) {
     ...
     for (elements_vector::iterator iter = m_children.begin();
	  iter != m_children.end(); iter++) {
	  if (!(*iter)->m_skip
	      && (*iter)->m_el_position != element_position_absolute) {
     ...
     for (elements_vector::iterator iter = m_absolutes.begin();
	  iter != m_absolutes.end(); iter++) {
	  if ((*iter)->on_mouse_over(x - m_pos.x, y - m_pos.y)) {
	       ret = true;
	  }
     }
...
bool litehtml::element::find_styles_changes(position::vector& redraw_boxes,
     ...
     for (used_selector::vector::iterator iter = m_used_styles.begin();
	  iter != m_used_styles.end() && !apply; iter++) {
     ...
     for (elements_vector::iterator i = m_children.begin();
	  ...
	  if (!(*i)->m_skip && (*i)->m_el_position != element_position_absolute) {
	       if ((*i)->find_styles_changes(redraw_boxes, x + m_pos.x,
					     y + m_pos.y)) {
		    ret = true;
	       }
	  }
...
const wchar_t* litehtml::element::get_cursor() {
     ...
     if (std::find(m_pseudo_classes.begin(), m_pseudo_classes.end(), L"hover")
...
void litehtml::element::init_font() {
     ...
     const wchar_t* decoration = get_style_property(L"text-decoration", true,
						    L"none");
...
void litehtml::element::draw_background(uint_ptr hdc, int x, int y,
					const position* clip) {
     position pos = m_pos;
     pos.x += x;
     pos.y += y;
	  ...
	  if (el_pos.does_intersect(clip)) {
	       ...
	       if (bg.m_color.alpha) {
		    m_doc->container()->fill_rect(hdc, bg_draw_pos, bg.m_color,
						  m_css_borders.radius);
	       }
	       if (!bg.m_image.empty()) {
		    m_doc->container()->draw_background(hdc, bg.m_image.c_str(),
							bg.m_baseurl.c_str(), bg_draw_pos, bg.m_position,
							bg.m_repeat, bg.m_attachment);
	       }
...
int litehtml::element::render_inline(litehtml::element* container,
				     int max_width) {
     int ret_width = 0;
     int rw = 0;
     for (elements_vector::iterator i = m_children.begin();
...
int litehtml::element::place_element(element* el, int max_width) {
     ...
     if (el->m_float == float_left) {
	  el->render(get_line_left(m_lines.back()->get_top()),
		     m_lines.back()->get_top(), m_lines.back()->line_right());
	  ...
	  if (m_lines.back()->get_left() + el->width()
	      > m_lines.back()->line_right()) {
	       int new_top = find_next_line_top(el->top(), el->width(), max_width);
	       el->m_pos.x = get_line_right(new_top, m_lines.back()->line_right())
	  ...
	  add_float(el);
	  fix_line_width(max_width);
	  ret_width = m_lines.back()->line_left()
	       + (max_width - m_lines.back()->line_right());
     } else {
	  if (el->is_break()) {
	       ...
	       case display_list_item: {
		    add_line(max_width, el->m_clear);
		    ret_width = el->render(m_lines.back()->get_left(),
					   m_lines.back()->get_top(),
					   m_lines.back()->line_right()
					   - m_lines.back()->get_left());
		    m_lines.back()->add_element(el);
		    add_line(max_width);
	       }
		    break;
	       ...
	       case display_block: {
		    ...
		    if (!el->is_floats_holder()) {
			 ret_width = el->render(0, m_lines.back()->get_top(),
						max_width);
			 m_lines.back()->add_element(el);
		    } else {
			 ret_width = el->render(m_lines.back()->get_left(),
						m_lines.back()->get_top(), max_width);
	       ...
	       case display_inline_block: {
		    el->render(m_lines.back()->get_left(),
			       m_lines.back()->get_top(),
			       m_lines.back()->line_right());
		    place_inline(el, max_width);
		    ret_width = el->right();
...
bool litehtml::element::set_pseudo_class(const wchar_t* pclass, bool add) {
     if (add) {
	  ...
	  string_vector::iterator pi = std::find(m_pseudo_classes.begin(),
						 m_pseudo_classes.end(), pclass);
...
bool litehtml::element::collapse_top_margin() const {
	 ...
	 && m_float == float_none && m_margins.top >= 0 && m_parent) {
	  return true;
     }
     return false;
}
...
bool litehtml::element::collapse_bottom_margin() const {
	 ...
	 && m_float == float_none && m_margins.bottom >= 0 && m_parent) {
	  return true;
     }
     return false;
}
