#include "litehtml.h"
#include "tokenizer.h"
#include "wx_container.h"


litehtml::wx_container::wx_container()
{
     m_hClipRgn = NULL;
}
...
litehtml::uint_ptr litehtml::wx_container::create_font( const wchar_t* faceName, int size, int weight, font_style italic, unsigned int decoration )
{
...
void litehtml::wx_container::delete_font( uint_ptr hFont )
{
     DeleteObject((HFONT) hFont);
}

int litehtml::wx_container::line_height( uint_ptr hdc, uint_ptr hFont )
{
...
int litehtml::wx_container::text_width( uint_ptr hdc, const wchar_t* text, uint_ptr hFont )
{
     HFONT oldFont = (HFONT) SelectObject((HDC) hdc, (HFONT) hFont);
     SIZE sz = {0, 0};
...
void litehtml::wx_container::draw_text( uint_ptr hdc, const wchar_t* text, uint_ptr hFont, litehtml::web_color color, const litehtml::position& pos )
{
     apply_clip((HDC) hdc);
...
void litehtml::wx_container::fill_rect( uint_ptr hdc, const litehtml::position& pos, const litehtml::web_color color, const litehtml::css_border_radius& radius )
{
     apply_clip((HDC) hdc);
...
litehtml::uint_ptr litehtml::wx_container::get_temp_dc()
{
     return (litehtml::uint_ptr) GetDC(NULL);
}

void litehtml::wx_container::release_temp_dc( uint_ptr hdc )
{
     ReleaseDC(NULL, (HDC) hdc);
}

int litehtml::wx_container::pt_to_px( int pt )
{
     HDC dc = GetDC(NULL);
...
int litehtml::wx_container::get_text_base_line( uint_ptr hdc, uint_ptr hFont )
{
     HDC dc = (HDC) hdc;
     if(!dc)
     {
...
void litehtml::wx_container::draw_list_marker( uint_ptr hdc, list_style_type marker_type, int x, int y, int height, const web_color& color )
{
     apply_clip((HDC) hdc);
...
void litehtml::wx_container::load_image( const wchar_t* src, const wchar_t* baseurl )
{
     std::wstring url;
...
void litehtml::wx_container::get_image_size( const wchar_t* src, const wchar_t* baseurl, litehtml::size& sz )
{
     std::wstring url;
     make_url(src, baseurl, url);
...
void litehtml::wx_container::draw_image( uint_ptr hdc, const wchar_t* src, const wchar_t* baseurl, const litehtml::position& pos )
{
     apply_clip((HDC) hdc);
...
void litehtml::wx_container::clear_images()
{
...
int litehtml::wx_container::get_default_font_size()
{
     return 16;
}

void litehtml::wx_container::draw_background( uint_ptr hdc, const wchar_t* image, const wchar_t* baseurl, const litehtml::position& draw_pos, const litehtml::css_position& bg_pos, litehtml::background_repeat repeat, litehtml::background_attachment attachment )
{
     apply_clip((HDC) hdc);
...
wchar_t litehtml::wx_container::toupper( const wchar_t c )
{
     return toupper(c);
}

wchar_t litehtml::wx_container::tolower( const wchar_t c )
{
     return tolower(c);
}

void litehtml::wx_container::set_clip( const litehtml::position& pos, bool valid_x, bool valid_y )
{
...
void litehtml::wx_container::del_clip()
{
     if(!m_clips.empty())
     {
	  m_clips.pop_back();
...
void litehtml::wx_container::apply_clip(HDC hdc)
{
     if(m_hClipRgn)
     {
	  DeleteObject(m_hClipRgn);
...
void litehtml::wx_container::release_clip(HDC hdc)
{
