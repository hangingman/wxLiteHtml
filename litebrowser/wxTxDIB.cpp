/**
 * wxTxDIB.cpp - stub implement class of TxDIB
 *
...
wxCTxDIB::wxCTxDIB( wxCTxDIB& val ) {
     m_bits		= NULL;
     m_width		= 0;
     m_height	= 0;
     m_maxAlpha	= 255;
...
wxCTxDIB::wxCTxDIB( LPCWSTR fileName ) {
     m_bits		= NULL;
     m_width		= 0;
     m_height	= 0;
     m_maxAlpha	= 255;
...
BOOL wxCTxDIB::load( LPCWSTR fileName ) {
     BOOL ret = FALSE;
...
BOOL wxCTxDIB::load( HRSRC hRes, HMODULE hModule /*= NULL*/ ) {
     BOOL	ret	= FALSE;
...
BOOL wxCTxDIB::load( LPBYTE data, DWORD size ) {
     BOOL ret = FALSE;
...
BOOL wxCTxDIB::draw( HDC hdc, int x, int y, int cx /*= -1*/, long cy /*= -1*/ ) {
     if(!m_bits || !cx || !cy) {
	  return FALSE;
     }
...
void wxCTxDIB::setTransColor( COLORREF clr ) {
     RGBQUAD rgb;
...
void wxCTxDIB::resample( int newWidth, int newHeight, wxCTxDIB* dst ) {
...
RGBQUAD wxCTxDIB::GetPixelColorInterpolated( float x, float y ) {
...
void wxCTxDIB::PreMultiplyWithAlpha() {
     if(!isValid()) {
	  return;
     }
...
void wxCTxDIB::_copy( wxCTxDIB& val ) {
     _copy(val.m_bits, val.m_width, val.m_height, TRUE);
     m_maxAlpha = val.m_maxAlpha;
}

void wxCTxDIB::crop( int left, int top, int right, int bottom, wxCTxDIB* dst /*= NULL*/ ) {
...
void wxCTxDIB::_copy( LPRGBQUAD newBits, int newWidth, int newHeight, BOOL copyBits ) {
...
void wxCTxDIB::tile( HDC hdc, LPRECT rcDraw, LPRECT rcClip /*= NULL*/ )
{
...
HBITMAP wxCTxDIB::createBitmap( HDC hdc )
{
     HBITMAP bmp = NULL;
...
BOOL wxCTxDIB::createFromHBITMAP( HBITMAP bmp ) {
     // FIBITMAP *dib = NULL;
     // if(bmp) 
     // {
     // 	  BITMAP bm;
...
BOOL wxCTxDIB::attach( LPVOID pdib ) {
...
BOOL wxCTxDIB::createFromHICON( HICON ico )
{
     destroy();
     BOOL ret = FALSE;
     if (ico) 
...
void wxCTxDIB::colorize( COLORREF clr )
{
     if(m_bits)
     {
...
RGBQUAD wxCTxDIB::GetAreaColorInterpolated( float const xc, float const yc, float const w, float const h ) {
     RGBQUAD color;      //calculated colour

     //area is wider and/or taller than one pixel:
     wxCTxDibRect2 area(xc-w/2.0f, yc-h/2.0f, xc+w/2.0f, yc+h/2.0f);   //area
     ...
     if (h>1 && w>1) {
	  ...
	  wxCTxDibRect2 intBL, intTR;     //bottom left and top right intersection
	  intBL=area.CrossSection(wxCTxDibRect2(((float)xi1)-0.5f, ((float)yi1)-0.5f, ((float)xi1)+0.5f, ((float)yi1)+0.5f));
	  intTR=area.CrossSection(wxCTxDibRect2(((float)xi2)-0.5f, ((float)yi2)-0.5f, ((float)xi2)+0.5f, ((float)yi2)+0.5f));
	  ...
	  wxCTxDibRect2 intersect;                                          //intersection with current pixel
	  wxCTxDibPoint2 center;
	  for (y=yi1; y<=yi2; y++) {
	       for (x=xi1; x<=xi2; x++) {
		    intersect = area.CrossSection(wxCTxDibRect2(((float)x)-0.5f, ((float)y)-0.5f, ((float)x)+0.5f, ((float)y)+0.5f));
...
void wxCTxDIB::resample2( int newWidth, int newHeight, wxCTxDIB* dst /*= NULL */ ) {
...
bool wxCTxDIB::QIShrink( int newWidth, int newHeight, wxCTxDIB* dst /*= NULL */ )
{
...
BOOL wxCTxDIB::savePNG( LPCWSTR fileName ) {
...
BOOL wxCTxDIB::saveJPG( LPCWSTR fileName, int quality /*= JPEG_QUALITY_GOOD*/ ) {
     // FIBITMAP* dib = FreeImage_Allocate(m_width, m_height, 32);
     // memcpy(FreeImage_GetBits(dib), m_bits, m_width * m_height * 4);
...
BOOL wxCTxDIB::saveBMP( LPCWSTR fileName ) {
     // FIBITMAP* dib = FreeImage_Allocate(m_width, m_height, 32);
     // memcpy(FreeImage_GetBits(dib), m_bits, m_width * m_height * 4);
...
BOOL wxCTxDIB::calcAlpha( wxCTxDIB* imgWhite, wxCTxDIB* imgBlack ) {
...
void wxCTxDIB::rotateLeft( wxCTxDIB* dst /*= NULL*/ ) {
     if(!isValid()) return;
...
void wxCTxDIB::rotateRight( wxCTxDIB* dst /*= NULL*/ ) {
     if(!isValid()) return;
...
BOOL wxCTxSkinDIB::load( LPCWSTR fileName, MARGINS* mg, BOOL tileX, BOOL tileY ) {
     wxCTxDIB dib;
...
BOOL wxCTxSkinDIB::load( wxCTxDIB* dib, MARGINS* mg, BOOL tileX, BOOL tileY )
{
     if(!dib)	return FALSE;
...
void wxCTxSkinDIB::draw( HDC hdc, LPRECT rcDraw, LPRECT rcClip ) {
     RECT rcPart;
     RECT rcTmp;
     if(m_margins.cxLeftWidth) {
	  if(m_margins.cyTopHeight) {
...
wxCTxDibSet::wxCTxDibSet( wxCTxDIB* img, int rows, int cols ) {
     m_cols = cols;
     m_rows = rows;
